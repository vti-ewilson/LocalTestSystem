...
namespace VTIWindowsControlLibrary.Classes.Util
{
    ...
    ///</summary>
    public static class VtiBitConverter
    {
        /// <summary>
        ...
        ///
        /// Returns:
        ///     An array of bytes with length numbytes.
        ///</summary>
        public static byte[] GetBytes(int value, int numbytes)
        {
            if (IsLittleEndian)
            {
                return System.BitConverter.GetBytes(value).Take(numbytes).ToArray();
            }
            else
            {
                return System.BitConverter.GetBytes(value).Take(numbytes).Reverse().ToArray();
            }
        }
        ...
        /// <summary>
        /// Returns a 24-bit signed integer converted from three bytes at a specified
        /// position in a byte array.
        /// </summary>
        /// <param name="value">An array of bytes.</param>
        /// <param name="startIndex">The starting position within value.</param>
        /// <returns>A 24-bit signed integer formed by four bytes beginning at startIndex.</returns>
        public static Int24 ToInt24(byte[] value, int startIndex)
        {
            byte[] bytes = new byte[4];
            if (IsLittleEndian)
            {
                value.Skip(startIndex).Take(3).ToArray().CopyTo(bytes, 0);
            }
            else
            {
                value.Skip(startIndex).Take(3).ToArray().CopyTo(bytes, 1);
            }
