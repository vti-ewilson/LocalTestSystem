...
namespace VTIWindowsControlLibrary.Classes.Configuration
{
    ...
    public class TorrconIVSerialPortParameter : EditCycleParameter<TorrconIVSerialPortSettings>
    {
        #region Fields (6)
        ...
        private StringParameterControl TorrconIVNameControl;
        ...
        ///  <param name="TorrconIVname">TCIV Name</param>
        ...
        public TorrconIVSerialPortParameter(string displayName, string TorrconIVname, string portName, int baudRate, Parity parity, int dataBits, StopBits stopBits, Handshake handshake, string toolTip)
        {
            DisplayName = displayName;
            ProcessValue = new TorrconIVSerialPortSettings(portName, TorrconIVname, baudRate, parity, dataBits, stopBits, handshake);
            ToolTip = toolTip;
            SequenceStep = "";
            OperatingSequence = "";
        }
        ...
        public TorrconIVSerialPortParameter(string displayName, string portName, string TorrconIVname, int baudRate, Parity parity, int dataBits, StopBits stopBits, Handshake handshake, string toolTip, string sequenceStep, string operatingSequence)
        {
            DisplayName = displayName;
            ProcessValue = new TorrconIVSerialPortSettings(portName, TorrconIVname, baudRate, parity, dataBits, stopBits, handshake);
            ToolTip = toolTip;
        ...
        /// </remarks>
        public TorrconIVSerialPortParameter()
        {
        }
        ...
        public override TreeNode CreateTreeNode(string nodeName)
        {
            ...
            {
                NewValue = new TorrconIVSerialPortSettings(
                    ProcessValue.PortName,
                    ProcessValue.TorrConIVName,
            ...
            treeNode2 = new TreeNode();
            treeNode2.Name = "Torrconame";
            treeNode2.Text = "Torrcon Name";
            editorTreeNode.Nodes.Add(treeNode2);
        ...
        public override Control GetEditorControl(PropertyInfo propertyInfo)
        {
            ...
            //"portNames.Where(x => x.Contains(NewValue.PortName)).FirstOrDefault()" MUST match an item contained in portNameEditorControl.Items for the value to be assigned to portNameEditorControl.Value.
            if (portNames.Where(x => x.Contains(NewValue.PortName)).FirstOrDefault() != null)
            {
                portNameEditorControl.Value = portNames.Where(x => x.Contains(NewValue.PortName)).FirstOrDefault();
            }
            else
            {
        ...
        public override Control GetEditorControl(PropertyInfo propertyInfo, string childNodeName)
        {
            switch (childNodeName)
            {
                ...
                case "Torrconame":
                    if (TorrconIVNameControl == null)
                    {
                        TorrconIVNameControl = new StringParameterControl();
                        TorrconIVNameControl.Description = ToolTip;
                        TorrconIVNameControl.ValueChanged += TorrconIVNameControl_ValueChanged;   
                    }
                    return TorrconIVNameControl;
        ...
        private void TorrconIVNameControl_ValueChanged(object sender, EventArgs<string> e)
        {
            NewValue.TorrConIVName = e.Value;
            if (TorrconIVname != null)
                ProcessValue.TorrConIVName = e.Value;
            TorrconIVname = NewValue.TorrConIVName;
        }
        ...
        public override string UpdateProcessValueFromNewValue()
        {
            ...
            if (ProcessValue != null)
            {
                ...
                sb.AppendLine(string.Format("        Torrcon Name: {0}", ProcessValue.TorrConIVName));
            ...
            sb.AppendLine(string.Format("        Torrcon Name: {0}", NewValue.TorrConIVName));
