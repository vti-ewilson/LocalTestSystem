...
namespace VTIWindowsControlLibrary.Classes.IO.SerialIO
{
    ...
    /// </summary>
    public partial class InficonLDS2010BinaryMDB : SerialIOBase
    {
        #region Enums
...
#pragma warning restore 1591
        ...
        /// </summary>
        public InficonLDS2010BinaryMDB()
        {
            InitializeComponent();
            Init();
        }
        ...
        public InficonLDS2010BinaryMDB(IContainer container)
        {
            container.Add(this);
        ...
        public override void Process()
        {
            if (!_DoNotCall)
            {
                ...
                try
                {
                    ...
                    _bytearray[0] = 5; //always
                    _bytearray[1] = 5; // len in bytes
                    _bytearray[2] = 99; // cmd
                    _bytearray[3] = (byte)leakRateUnits; // data
                    _bytearray[4] = Convert.ToByte(_bytearray[0] + (byte)_bytearray[1] + (byte)_bytearray[2] + (byte)_bytearray[3]); // chk sum
                    ...
                    _DoNotCall = false;
                    // retData[0] should be same as length 5
                    // [1] - command echo
                    if (retData[1] == _bytearray[2])
                    {
                        leakRate = BitConverter.ToSingle(new byte[] { retData[4], retData[5], retData[3], retData[2] }, 0);
                    }
                    //else
                    //{
                    //    leakRate = 0f;  // checking for errors
                    //}
                ...
                try
                {
                    ...
                    _bytearray[1] = 5;
                    _bytearray[2] = 2;
                    _bytearray[3] = (byte)pressureUnits;
                    _bytearray[4] = Convert.ToByte(_bytearray[0] + (byte)_bytearray[1] + (byte)_bytearray[2] + (byte)_bytearray[3]);
                    serialPort1.Write(_bytearray, 0, 5);
                    Thread.Sleep(100);
                    Actions.WaitForUpTo(3000, () => this.serialPort1.BytesToRead > 2);
                    byte[] retData = null;
                    ...
                    if (retData[1] == 2)
                    {
                        pressure = BitConverter.ToSingle(new byte[] { retData[4], retData[5], retData[3], retData[2] }, 0);
                    }
                    else
                    {
                        pressure = 110f;
