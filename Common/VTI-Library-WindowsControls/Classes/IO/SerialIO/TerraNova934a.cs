...
namespace VTIWindowsControlLibrary.Classes.IO.SerialIO
{
    ...
    /// </summary>
    public class TerraNova934A : SerialIOBase
    {
        #region Public Enum

        /// <summary>
        /// Enumeration of status register values
        ///       signal.TerraNova934FilamentStatus.Value = Convert.ToDouble(IO.SerialIn.TerraNova934Control.GetIonGaugeParameter((int)TerraNova934Mode.FilamentStatusAutofilament));
        /// </summary>
        ...
        /// </summary>
        public TerraNova934A(Single Min, Single Max, String Units)
            : base()
        {
            _min = Min;
            _max = Max;
            ...
            pressureSignalA = new AnalogSignal("Gauge A Press", "Torr", "0.0E-0", 1000, false, true);
            pressureSignalB = new AnalogSignal("Gauge B Press", "Torr", "0.0E-0", 1000, false, true);
            pressureIonGauge = new AnalogSignal("Ion Gauge Press", "Torr", "0.0E-0", 1000, false, true);
            Format = "0.0E-0";
        }
        ...
        /// <summary>
        /// Get Ion Gauge Pressure from Terranova 934
        /// </summary>
        ///
                ...
                try
                {
                    ...
                    if (!strDummy.Contains("999"))
                    {
                        if (strDummy == "0" || strDummy == "") // || _filamentStatus == 0)
                        {
                            ...
                            //Console.WriteLine("Terranova 934 IG Error '" + strDummy + "'");
                            IGErrorReads++;
                            if (IGErrorReads > 6)
                                fRet = 1000;
                            else
                        ...
                        {
                            IGErrorReads = 0;
                            if (strDummy.Substring(0, 1) == "-")
                            {
                                strMan = strDummy.Substring(0, 3);
                                strExp = strDummy.Substring(3);
                            }
                            else
                            {
                                strMan = strDummy.Substring(0, 2);
                                strExp = strDummy.Substring(2);
                            }
                            bool result = float.TryParse(strMan, out fMan);
                            if (result == true)
                                result = float.TryParse(strExp, out fExp);

                            if (result == true)
                            {
                                //float fMan = Convert.ToSingle(strMan);
                                //float fExp = Convert.ToInt32(strExp);
        ...
        /// <summary>
        /// Get Gauge A Pressure from Terranova 934
        /// </summary>
        ...
        /// <summary>
        /// Get Gauge B Pressure from Terranova 934
        /// </summary>
        ...
        private string IonGaugeStart()
        {
            ...
            try
            {
                strDummy = ProcessCommandAndReturnValue("A");
                sRet = strDummy;
                return sRet;
            }
            catch (Exception e)
        ...
        private string IonGaugeStop()
        {
            ...
            try
            {
                strDummy = ProcessCommandAndReturnValue("B");
                sRet = strDummy;
                return sRet;
            }
            catch (Exception e)
        ...
        public override void Process()
        {
            if (Monitor.TryEnter(this.SerialLock, 2000))
            {
                ...
                catch (Exception e)
                {
                    ...
                    VtiEvent.Log.WriteError("Error inside Terra934 | Process", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
                    //commError = true;
                }
                finally
                {
        ...
        /// <summary>
        /// Processes commands to the Terra934 Controller
        /// </summary>
        private string ProcessCommandAndReturnValue(string strCommand)
        {
            ...
            {
                VtiEvent.Log.WriteError("Error inside Terranova 934 ProcessCommandAndReturnValue, Serial Port Write", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
                _DoNotCall = false;
            ...
            {
                VtiEvent.Log.WriteError("Error inside  Terranova 934 ProcessCommandAndReturnValue, Serial Port Read", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
                _DoNotCall = false;
                return "0"; // Read error
            }
        ...
        private string ProcessRegisterAndReturnValues(string strCommand)
        {
            ...
            {
                VtiEvent.Log.WriteError("Error inside Terranova 934 ProcessRegisterAndReturnValues, Serial Port Write", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
                _DoNotCall = false;
                return "-1"; // write error
            }
            // Read back response
            ...
            {
                VtiEvent.Log.WriteError("Error inside  Terranova 934 ProcessRegisterAndReturnValues, Serial Port Read", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
                _DoNotCall = false;
        ...
        public string SendManualCommand(string strCommand)
        {
            ...
            {
                VtiEvent.Log.WriteError("Error inside  Terranova 934 SendManualCommand", VTIWindowsControlLibrary.Enums.VtiEventCatType.Serial_IO, e.Message);
            }
            return "";
        }
