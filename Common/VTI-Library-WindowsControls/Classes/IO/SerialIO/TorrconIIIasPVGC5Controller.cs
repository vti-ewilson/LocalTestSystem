...
namespace VTIWindowsControlLibrary.Classes.IO.SerialIO
{
    ...
    /// </summary>
    public class TorrconIIIAsPVGC5Controller : SerialIOBase
    {
        ...
        /// </summary>
        public TorrconIIIAsPVGC5Controller(Single Min, Single Max, String Units)
          : base()
        {
            _min = Min;
            _max = Max;
        ...
        public override void Process()
        {
            //if (Monitor.TryEnter(this.SerialLock, 500))
            {
                try
                {
                    ...
                    if (TempString.Contains("EPH"))
                    {
                        _pressure = 999.9;
                    }
                    else
                    {
                        if (TempString.Contains("EPL"))
                        {
                            _pressure = 0.000;
                        }
                        else
                        {
                            _pressure = Convert.ToDouble(TempString);
                        }
                    }
